...
[Context management](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/skills>)
...
For ready-to-use skills that improve your agent’s performance on LangChain ecosystem tasks, see the [LangChain Skills](https://docs.langchain.com/oss/python/deepagents/<https:/github.com/langchain-ai/langchain-skills>) repository.
...
Deep agent skills follow the [Agent Skills specification](https://docs.langchain.com/oss/python/deepagents/<https:/agentskills.io/specification>).
...
This example uses `FilesystemBackend` to load skills from disk. For other storage options, including loading skills from remote sources, see Backends and remote skill loading.Point each source path at a directory that contains skill directories. A path that points directly at a skill directory with `SKILL.md` is not loaded.
...
In Deep Agents, [`SkillsMiddleware`](https://docs.langchain.com/oss/python/deepagents/<https:/reference.langchain.com/python/deepagents/middleware/skills/SkillsMiddleware>) (part of the [Deep Agents stack](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/customization#deep-agents-stack>) when you pass `skills`) handles the first two levels, with the third level being handled by the LLM:
...
Use the [`skills-ref` validation tool](https://docs.langchain.com/oss/python/deepagents/<https:/github.com/agentskills/agentskills/tree/main/skills-ref>) to check that your `SKILL.md` frontmatter follows the Agent Skills specification naming and format conventions.
...
The SDK only loads the sources you pass in `skills`. It does not automatically scan CLI directories such as `~/.deepagents/...` or `~/.agents/...`.For CLI storage conventions, see [App data](https://docs.langchain.com/oss/python/deepagents/</oss/deepagents/code/configuration#data-locations>).
...
This pattern is useful when different users or tenants need fully independent skill libraries that can be updated separately. For a managed solution that handles skill access, sharing, and workspace-level visibility out of the box, see [Fleet skills](https://docs.langchain.com/oss/python/deepagents/</langsmith/fleet/skills>).
...
When you use [subagents](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/subagents>), you can configure which skills each type has access to:
...
For more information on subagent configuration and skills inheritance, see [Subagents](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/subagents>).
...
Production deployments usually need to control three things: which skills each user can see, whether the agent can modify skill files, and whether writes require human approval. You control visibility with the `skills` argument and backend routing, access with [filesystem permissions](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/permissions>), and approval with [`interrupt_on`](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/human-in-the-loop>) or permission rules with `mode="interrupt"`.
...
If agents may write to skill files but you want a human in the loop first, use either [`interrupt_on`](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/human-in-the-loop>) or a permission rule with `mode="interrupt"`. Both pause before `write_file` or `edit_file` runs and use the same resume flow.
[code] 
    from deepagents import FilesystemPermission, create_deep_agent
    from langgraph.checkpoint.memory import MemorySaver
...
Alternatively, configure `interrupt_on={"write_file": True, "edit_file": True}` to require approval for all filesystem writes, not only skills paths. See [Human-in-the-loop](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/human-in-the-loop>) for handling and resuming interrupts.
...
By default, agents can write to skill files if the backend permits it and no permission rule blocks the path. To let agents create or refine skills without touching shared libraries:
  ...
  3. Do not add a `deny` rule for the writable path. Place more specific rules before broader deny rules if you mix shared and personal paths ([rule ordering](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/permissions#rule-ordering>)).
...
Without code execution, skills are passive: the agent reads instructions and follows them using its available tools. Code execution turns skills into active capabilities. A skill can ship a tested script that calls an API, transforms data, validates output, or runs a pipeline — and the agent executes it deterministically rather than regenerating the logic from instructions each time. This is especially valuable for workflows that require exact behavior (data transformations, API integrations, compliance checks) or that depend on libraries the agent cannot use through tool calls alone. Skills execute code through sandbox scripts: the agent runs a bundled script when it needs to install dependencies, run tests, call CLIs, or work with an operating-system filesystem.
...
The agent can _read_ scripts from any backend, but to _execute_ them, the agent needs access to a shell, which only [sandbox backends](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/sandboxes>) provide. [Sandbox backends](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/sandboxes>) run in isolated containers. Skill files stored outside the sandbox are not available inside it, which means the agent cannot execute skill scripts or access skill resources unless they are transferred in first. Use [custom middleware](https://docs.langchain.com/oss/python/deepagents/</oss/python/langchain/middleware/custom>) to handle this transfer:
...
For a complete example that seeds both skills and memories before execution and syncs both back afterward, see [syncing skills and memories with custom middleware](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/going-to-production#example-syncing-skills-and-memories-with-custom-middleware>).
...
**Problem** : The agent does not list a skill in its system prompt, or `read_file` on `SKILL.md` fails. **Solutions** :
  ...
  3. **Validate`SKILL.md` frontmatter.** The `name` must match the parent directory name and follow the [Agent Skills specification](https://docs.langchain.com/oss/python/deepagents/<https:/agentskills.io/specification>). Use the [`skills-ref` validation tool](https://docs.langchain.com/oss/python/deepagents/<https:/github.com/agentskills/agentskills/tree/main/skills-ref>) to check formatting.
  4. **Check file size.** Deep Agents skips `SKILL.md` files over 10 MB during discovery.
  5. **Check layered sources.** When the same skill name appears in multiple sources, the last source wins. An older or empty skill from a later path can override the one you expect.
...
**Problem** : The agent reads `SKILL.md` but cannot access scripts, references, or assets. **Solutions** :
  ...
  3. **Sync skills into sandboxes.** If you use [sandbox backends](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/sandboxes>), skill files outside the container are not available until you copy them in. See Sandbox scripts and [syncing skills and memories with custom middleware](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/going-to-production#example-syncing-skills-and-memories-with-custom-middleware>).
...
**Problem** : The agent reads a script but cannot run it. **Solution** : The agent can read scripts from any backend, but running them requires a [sandbox backend](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/sandboxes>). See Execute code with skills.
...
Skills, [memory](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/memory>) (`AGENTS.md` files), and tools all provide context or capabilities to the agent. The following table summarizes when to reach for each:
...
The [Agent Skills specification](https://docs.langchain.com/oss/python/deepagents/<https:/agentskills.io/specification>) defines the following frontmatter fields:
...
Refer to the full [Agent Skills specification](https://docs.langchain.com/oss/python/deepagents/<https:/agentskills.io/specification>) for detailed constraints and validation rules. In Deep Agents, `SKILL.md` files must be under 10 MB. Files exceeding this limit are skipped during skill loading.

For more example skills, see [Deep Agents example skills](https://docs.langchain.com/oss/python/deepagents/<https:/github.com/langchain-ai/deepagents/tree/main/libs/code/examples/skills>).
...
[Edit this page on GitHub](https://docs.langchain.com/oss/python/deepagents/<https:/github.com/langchain-ai/docs/edit/main/src/oss/deepagents/skills.mdx>) or [file an issue](https://docs.langchain.com/oss/python/deepagents/<https:/github.com/langchain-ai/docs/issues/new/choose>).

Was this page helpful?

YesNo

[StreamingPrevious](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/streaming>)[MemoryNext](https://docs.langchain.com/oss/python/deepagents/</oss/python/deepagents/memory>)
