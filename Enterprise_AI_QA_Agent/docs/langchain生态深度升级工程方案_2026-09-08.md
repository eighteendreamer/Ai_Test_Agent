...
- 可选门禁：`RUN_LIVE_POSTGRES_SOAK_MAX_ERROR_RATE`、`MAX_COMPLETE_P95_MS`、`MAX_P95_GROWTH_RATIO`、`MAX_RSS_GROWTH_BYTES`、`MAX_CONNECTIONS` 均默认为观察模式；只有项目方在 `.env` 明确配置后才阻断。错误率使用 `-1` 表示禁用，避免把未确认的示例阈值伪装成正式 SLA。
...
当前测试结果：适配器、消息转换、工具转换、Flag 分流、Legacy 边界、Structured Output、错误映射、取消传播和双跑契约专项 `21 passed`；后端全量 `777 passed, 10 skipped, 1 warning`（24.45s）；`python -m compileall -q src tests` 通过。`pip check` 仍有环境既有的 browser-use/mitmproxy 等版本冲突，本批未新增可归因冲突。
...
### DA-E3 认知计划、同步子代理与单轮取消实施记录（已完成，2026-09-10）

| 项目 | 结果 |
|---|---|
| 当前状态 | 已完成；认知 todo、受控同步子代理、typed events、单轮取消/超时和 LangSmith 父子 Trace 均已验收 |
...
| 子代理边界 | 仅在 `DEEP_AGENTS__COGNITIVE_SUBAGENTS_ENABLED=true` 时开放一个同步 `code-review-researcher`；显式 `tools=[]` 后仅由官方 `FilesystemMiddleware` 加入 `ls/read_file/glob/grep`，没有嵌套 `task`、写/删/execute 或业务工具。父 Agent 使用官方 `ToolCallLimitMiddleware(tool_name="task", run_limit=1)`，子代理使用官方 Model/ToolCallLimitMiddleware；`worker_dispatches=[]`，不进入 Coordinator |
...
| 配置 | `DEEP_AGENTS__TURN_TIMEOUT_SECONDS=600`；新增 `COGNITIVE_SUBAGENTS_ENABLED=false`、`MAX_SUBAGENT_CALLS_PER_TURN=1`、`SUBAGENT_MODEL_CALL_LIMIT=6`、`SUBAGENT_TOOL_CALL_LIMIT=12`，所有限额必须大于 0；`.env.example` 与本地 `.env` 已同步，配置中不记录或输出真实 Key |
...
| typed events | 0.7.13 没有最新文档中的 `agent.stream_events()` 投影对象，因此使用该版本真实存在的 LangGraph `astream_events(version="v2")`；以官方 `task` 的 `on_tool_start/on_tool_end` 映射 `graph.subagent_started/completed`，只记录 name/run_id/parent_run_id，不采集任务正文。进入外层 LangSmith Trace 后根事件会带 parent_ids，已用稳定 `run_name=enterprise_ai_qa_agent.deep_agent_turn` 提取最终 state，禁止假定根事件 parent_ids 为空 |
| 自动化测试 | 最终主环境定向（Deep Agents + Observability）：`37 passed, 14 skipped`；C4 定向：`51 passed`；C4 Deep Agents 单文件：`30 passed`；主环境全量：`797 passed, 25 skipped, 1 warning in 34.17s`；C4 全量：`812 passed, 10 skipped, 1 warning in 27.30s`；`compileall -q src` 通过 |
| 覆盖场景 | 官方 `write_todos`、todo 映射、官方同步 task、父/子工具面、委派/模型/工具限额装配、正常完成、总超时、`/interrupt`、客户端断开、监听收尾、取消清理、无残留任务、不可恢复投影、Deep Agents 外层 Observability、`worker_dispatches=[]`、旧 Runtime 全量回归 |
| 真实服务验证 | 正常会话 `02a1f501-da01-452b-9242-125c9de0deea`：HTTP 200、completed、messages=2、snapshot=1、subagent started/completed 各 1；主动中断会话 `db9262c1-3ffa-496b-9830-4ebb4bbf332e`：running → interrupt_requested → interrupted，1.5 秒内收敛，started=1/completed=0、interrupt_applied=1、snapshot=1、`is_resumable=false`。此前一次启动曾返回 401，随后同一数据库默认模型恢复并完成上述真实验证，因此 401 记录保留为历史环境波动，不再是当前阻塞 |
| LangSmith 真实验证 | 本地会话 `dc8e6335-9848-4550-98cf-962c7ec4a14e` 链接外部 root run `01a089af-36ea-7140-a9b2-e0e49df134a8`；官方 SDK 回读：root `enterprise_ai_qa_agent.turn`、一级 `enterprise_ai_qa_agent.deep_agent_turn`、下级 `task`、再下级 `code-review-researcher`，共用同一 trace_id，66 个子 Run、0 error。中断 root `01a089b2-dde3-7031-afd1-a9d35d353e7e` metadata 为 `outcome=interrupted/termination_reason=interrupted/control_state=interrupted`，避免把受控中断计入业务完成 |
...
### DA-E5 审批续跑跨 Worker 租约实施记录（进行中，2026-09-10）

| 项目 | 状态与证据 |
|---|---|
| 当前状态 | 已完成审批裁决后的 continuation 单所有者、续租、完成封存和崩溃后可重领；DA-E5 整体仍为进行中，通用 Session/turn fencing 尚未完成 |
| 失败先行 | 两个独立 `SessionService` 实例并发提交同一 approved 决策，原实现实际调用 `resume_after_approval` 两次，回归得到 `runtime.calls == 2`。根因是 Approval CAS 只保护 pending → approved/denied，没有保护裁决后的后台续跑调度 |
| 采用做法 | 在现有 Approval Store 的 `metadata` 上增加 continuation lease，不建新表/调度器：claim 写 owner/token/数据库到期时间；renew 和 complete 必须匹配 token；complete 删除活动租约并写 completed_at；已完成不可重领，到期租约可由新 Worker 接管 |
| PostgreSQL 原子性 | claim 是单条条件 `UPDATE ... RETURNING`，要求审批已裁决、未完成且不存在有效租约；续租和完成均带 fencing token 条件。到期时间使用数据库 `now()`，不依赖 Worker 时钟。InMemory Store 用既有锁实现同一协议 |
| 生命周期 | `SessionService` 只有领取成功才把 Session 置为 resuming 并创建后台任务；活动期间按配置心跳续租；租约丢失记录结构化 error 并调用现有 Runtime interrupt。正常终态或已持久化 interrupted 状态才封存 continuation；Task 被取消/进程退出不写 completed，保留租约等待到期恢复 |
| 配置 | 新增 `ORCHESTRATION__APPROVAL_CONTINUATION_LEASE_SECONDS=120`、`ORCHESTRATION__APPROVAL_CONTINUATION_HEARTBEAT_SECONDS=30`，已写入本地 `.env` 与 `.env.example`；校验 heartbeat 必须大于 0 且严格短于 lease。未新增密钥或依赖 |
| 事件与日志 | 只有领取者写 `approval.resolved` 和 `approval.continuation_scheduled`；竞争失败者写 `approval.continuation_already_claimed`。续租丢失和完成 token 失效分别记录可检索日志，不记录 token/owner 或审批参数 |
| 自动化测试 | 双实例失败用例修复后只调用一次 Runtime，且 resolved/already_claimed 事件各一；覆盖 heartbeat 丢失触发 interrupt、任务取消不误标 completed、配置时序校验。Deep Agents/审批专项最终 `44 passed`；相关 Session/审批链曾以 `108 passed` 验证 |
...
### DA-E5 执行中 Stage checkpoint 对账实施记录（进行中，2026-09-11）
...
#### PostgreSQL 10 分钟稳定性阶梯观察（2026-09-11）

- 命令：`RUN_LIVE_POSTGRES_SOAK=1 RUN_LIVE_POSTGRES_SOAK_SECONDS=600 RUN_LIVE_POSTGRES_SOAK_SAMPLE_INTERVAL_SECONDS=30 RUN_LIVE_POSTGRES_SOAK_ITERATION_INTERVAL_SECONDS=0.25 RUN_LIVE_POSTGRES_SOAK_WORKERS=4 pytest tests/test_live_postgres_capacity.py::test_live_postgres_lifecycle_soak -q -s`。
- 结果：`1 passed in 601.49s`；1969 次迭代全部完成，错误率 `0.000000`。
- 指标：完成 P50 `11.22ms`、P95 `20.12ms`、P99 `38.00ms`；首窗口 P95 `33.08ms`、末窗口 P95 `13.97ms`；RSS 样本 `16236544 -> 111194112`，测试报告净差 `-24576000` bytes；连接峰值 `4`。
...
| 测试 | Coordinator 专项 `51 passed`；主环境全量 `824 passed, 35 skipped`；C4 全量 `844 passed, 16 skipped`；真实 PostgreSQL 并发/接管与 Coordinator 恢复对账 `6 passed` |
