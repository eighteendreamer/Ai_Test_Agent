...
状态：进行中（当前主服务组合、真实 LangSmith 外部上报、Deep Agents C2/C4 候选组合均已有验证证据；正式长任务性能、主环境升级与回滚仍未完成）
...
| `langsmith` | 0.10.18 | 已验证（本地适配、SDK 协议与真实云端上报） | 观测契约、root→node 协议测试；真实默认模型会话的外部 Trace、父子树和敏感字段扫描已记录 |
...
2. 阶段 1 的核心 LangSmith 接入与真实上报已验收，但阶段整体仍进行中（以主工程方案最新台账为准）；LangSmith Key 已配置，DA-E4 已补齐真实工具父子 Trace。不能把历史“未配 Key”的记录当作当前阻塞。
...
## 13. 2026-09-11 Coordinator 子会话 dispatch claim 增量

- 失败先行证明相同 parent turn/task_id 的并发 dispatch 原先会创建两个 child session；新增 SessionStore 原子 `claim_coordinator_dispatch` 后，只有一个调用创建 child，其他调用返回 `status=duplicate`。
