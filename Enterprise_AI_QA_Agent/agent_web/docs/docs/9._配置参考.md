...
| `DATABASE__POSTGRES_TEST_CASE_EMBEDDING_TABLE` | agent_test_case_embeddings | 测试用例长期向量表 |
| `ORCHESTRATION__TEST_CASE_VECTOR_TOP_K` | 6 | 测试用例在线召回数量 |
| `ORCHESTRATION__TEST_CASE_VECTOR_CANDIDATE_MULTIPLIER` | 8 | Redis 候选到 PostgreSQL 权威复核的倍率 |
...
Embedding 维度不在配置文件中填写，由当前启用 `embedding_retrieval` 用途的模型健康检查动态探测；系统不截断、不补零，Redis 与 PostgreSQL 按 `embedding_version` 保存和隔离。
